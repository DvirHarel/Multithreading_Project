...
public abstract class Subscriber extends RunnableSubPub {
    ...
    private static MessageBroker messageB;
    ...
    public Subscriber(String name) {
            super(name);
            synchronized (this){
            messageB = MessageBrokerImpl.getInstance();
            terminated = false;
            callBackMap = new ConcurrentHashMap<Class, Callback>();
        }
    }
    /**
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        messageB.subscribeEvent(type,this);
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        messageB.subscribeBroadcast(type,this);
        callBackMap.putIfAbsent(type,callback);
    }
    ...
    public final void run() {
        messageB.register(this);
        initialize();
        //subscribeBroadcast();
        while (!terminated) {
            try{
                Message message = messageB.awaitMessage(this);
